...
<?php else: ?>
	<?php if($authPrefix == Configure::read('Routing.admin')): ?>
				<li><?php $bcBaser->link($bcBaser->getImg('admin/btn_logo.png', array('alt' => 'baserCMS管理システム', 'class' => '')), '/admin', array('title' => 'baserCMS管理システム')) ?></li>
	<?php else: ?>
...
<?php endif ?>
		...
		<div id="UserMenu">
			<ul class="clearfix">
				<li>
<?php if(!empty($user)): ?>
					<?php $bcBaser->link($user['real_name_1']." ".$user['real_name_2'].' '.$bcBaser->getImg('admin/btn_dropdown.png', array('width' => 8, 'height' => 11, 'class' => '')), 'javascript:void(0)', array('class' => 'title')) ?>
					<ul>
...
<?php elseif($this->name != 'Installations' && $this->params['url']['url'] != $loginUrl && !Configure::read('BcRequest.isUpdater')): ?>
					<?php $bcBaser->link('ログインしていません '.$bcBaser->getImg('admin/btn_dropdown.png', array('width' => 8, 'height' => 11, 'class' => '')), 'javascript:void(0)', array('class' => 'title')) ?>
					<ul>
...
<?php if(!empty($user) && $authPrefix == Configure::read('Routing.admin')): ?>
				<li>
					<?php $bcBaser->link('システムナビ'.' '.$bcBaser->getImg('admin/btn_dropdown.png', array('width' => 8, 'height' => 11, 'class' => '')), 'javascript:void(0)', array('class' => 'title')) ?>
